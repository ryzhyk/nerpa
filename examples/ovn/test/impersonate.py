...
def main():
    ...
    ovnParser = parglare.Parser(g, build_tree=True) # , debug=True)
    g = parglare.Grammar.from_string(ovsGrammar)
    ovsParser = parglare.Parser(g, build_tree=True)

    # TODO: update these with the location and names of the actual binaries
    currentParser = None
    originalCommand = None
    prefix = "/home/mbudiu/git/ovs/"
    if impersonate == "ovn-nbctl":
        originalCommand = prefix + "ovn/utilities/ovn-nbctl"
        currentParser = ovnParser
    else:
        originalCommand = prefix + "utilities/ovs-vsctl"
        currentParser = ovsParser
    ...
    if len(sys.argv) > 1:
        ...
        for arg in sys.argv[1:]:
            ...
            line += " " + arg
        try:
            parseOptions(currentParser, line)
